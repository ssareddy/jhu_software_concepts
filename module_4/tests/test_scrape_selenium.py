...
------------------------------
Real coverage for scrape.py's Selenium- and network-dependent functions:
...
Note on mock targets: scrape.py imports ChromeWebDriver directly from
...
def test_check_robots_txt_allows_survey_when_not_disallowed():
    ...
    with patch("scrape.urllib.request.urlopen", return_value=_mock_urlopen(ROBOTS_ALLOW)):
...
def test_check_robots_txt_disallows_when_survey_blocked():
    ...
    with patch("scrape.urllib.request.urlopen", return_value=_mock_urlopen(ROBOTS_DISALLOW)):
...
def test_check_robots_txt_ignores_named_bot_blocks():
    ...
    (browser UA) result, since scrape.py uses a standard browser UA."""
    with patch("scrape.urllib.request.urlopen", return_value=_mock_urlopen(ROBOTS_ALLOW)):
        assert scrape.check_robots_txt() is True
...
def test_check_robots_txt_wildcard_block_ends_at_named_agent():
    ...
    with patch("scrape.urllib.request.urlopen",
...
def test_build_driver_constructs_headless_chrome():
    ...
    with patch("scrape.ChromeWebDriver", return_value=fake_driver) as mock_chrome:
...
def test_get_page_source_success_first_attempt():
    ...
    with patch("scrape.WebDriverWait") as mock_wait, patch("scrape.time.sleep"):
...
def test_get_page_source_retries_then_succeeds():
    ...
    with patch("scrape.WebDriverWait") as mock_wait, patch("scrape.time.sleep") as mock_sleep:
...
def test_scrape_data_raises_when_robots_txt_disallows():
    ...
    with patch("scrape.check_robots_txt", return_value=False):
...
def test_scrape_data_happy_path_two_pages(tmp_path):
    ...
    with patch("scrape.check_robots_txt", return_value=True), \
         patch("scrape._build_driver", return_value=fake_driver), \
         patch("scrape._get_page_source", return_value="<html>content</html>"), \
         patch("scrape._parse_page", side_effect=fake_parse_page), \
         patch("scrape._safe_quit") as mock_quit, \
...
def test_scrape_data_pauses_and_rebuilds_on_rate_limit(tmp_path):
    ...
    with patch("scrape.check_robots_txt", return_value=True), \
         patch("scrape._build_driver", side_effect=build_calls), \
         patch("scrape._get_page_source", side_effect=get_page_results), \
         patch("scrape._parse_page", return_value=[{"raw_institution_program": "Test U"}]), \
         patch("scrape._safe_quit") as mock_quit, \
         patch("scrape.time.sleep") as mock_sleep:
...
def test_scrape_data_returns_in_memory_when_no_output_file():
    ...
    with patch("scrape.check_robots_txt", return_value=True), \
         ...
         patch("scrape._get_page_source", return_value="<html>content</html>"), \
         patch("scrape._parse_page", return_value=[{"raw_institution_program": "Test U"}]), \
         patch("scrape._safe_quit"), \
         patch("scrape._save_raw") as mock_save, \
         patch("scrape.time.sleep"):
...
def test_scrape_data_logs_loaded_existing_records(tmp_path, capsys):
    ...
    with patch("scrape.check_robots_txt", return_value=True), \
         patch("scrape._load_existing_records", return_value=existing), \
         patch("scrape._build_driver", return_value=fake_driver), \
         patch("scrape._get_page_source", return_value="<html>content</html>"), \
         patch("scrape._parse_page", return_value=[{"raw_institution_program": "New U"}]), \
         patch("scrape._safe_quit"), \
         patch("scrape.time.sleep"):
        records = scrape.scrape_data(max_pages=1, output_file=out_file, start_page=1)
...
def test_scrape_data_writes_checkpoint_every_50_pages(tmp_path):
    ...
    with patch("scrape.check_robots_txt", return_value=True), \
         patch("scrape._build_driver", return_value=fake_driver), \
         patch("scrape._get_page_source", return_value="<html>content</html>"), \
         patch("scrape._parse_page", side_effect=lambda _html: [{"raw_institution_program": "Test U"}]), \
         patch("scrape._safe_quit"), \
         patch("scrape.time.sleep"), \
         patch("scrape._save_raw") as mock_save:
