...
├── requirements.txt
├── .env.example                # Template for local Postgres credentials
├── writeup.pdf
...
├── db/
│   ├── init.sql                # Schema for the applicants table
│   ├── load_data.py            # Populates Postgres from a cleaned JSON file
│   └── db_config.py            # Standalone copy used by load_data.py
└── web/
    ├── app/
...
## 4. Set Up the Database (Required for the Analysis Page)

The existing Analysis page (`/`) queries a local PostgreSQL database. The
new "Will You Get In?" page does **not** need this — it only needs the
saved model files above — so this step can be skipped if you only care
about testing the prediction page.

**a. Create your credentials file.** Copy `.env.example` to a new file
named exactly `.env` in this folder, and fill in your real PostgreSQL
username/password:
...
`run.py` loads this file automatically at startup (via `python-dotenv`),
so no manual environment variable setup is needed. `.env` is intentionally
not committed to the repository, since it holds real credentials.

**b. Make sure PostgreSQL is running** with a `gradcafe` database matching
the `.env` values.

**c. Create the schema and load data**, if this database doesn't already
have the `applicants` table populated:
...
(Adjust the `--json` path to wherever your cleaned Grad Cafe dataset
actually lives.) This creates the table from `db/init.sql` if it doesn't
already exist, and inserts records, skipping duplicates by URL.

If you see a `permission denied for table applicants` error instead of an
authentication error, your `.env` user has connected successfully but
lacks privileges on the table (commonly happens if the table was created
under a different Postgres role). Fix with, as a superuser:
