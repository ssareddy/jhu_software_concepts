...
| `module_2/`  | Web Scraping Assignment                                    | Scraped 30,000+ Grad Cafe admissions entries with Selenium/BeautifulSoup, standardized program/university names with a self-hosted local LLM.                                                                                                               |
...
| `module_6/`  | Containerized Flask + PostgreSQL Analysis Website          | Docker Compose deployment of the Grad Cafe analysis site: Flask web service, PostgreSQL, and a RabbitMQ-backed worker for scraping/ETL.                                                                                                                     |
...
| `module_9/`  | KMeans Clustering of Graduate Programs                     | TF-IDF + PCA clustering of graduate program names, with an elbow analysis and animated Plotly visualizations.                                                                                                                                               |
| `module_10/` | Interactive Diamonds Pricing Dashboard                     | Interactive Plotly Dash dashboard analyzing diamond pricing against physical features.                                                                                                                                                                      |
| `module_11/` | MLOps Tracking for the KMeans Pipeline                     | MLflow and Weights & Biases experiment tracking added to the Module 9 clustering pipeline.                                                                                                                                                                  |
| `module_12/` | Two-Layer Neural Network From Scratch                      | Admissions classifier built entirely from scratch in NumPy (forward/backward propagation, no ML framework).                                                                                                                                                 |
| `module_13/` | Fine-Tuned Language Model Deployment                       | Fine-tuned DistilBERT admissions classifier, deployed as a live "Will You Get In?" Flask page.                                                                                                                                                              |
